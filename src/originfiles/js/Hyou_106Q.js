...
// CSVファイルへのパス
csv.open("GET", "Csv/Hyou_106Q.csv", false);
...
for( let i = 0; i < eigyobu_num.length; i++ ){
  for( let j = 1; j < csvArray.length; j++){
    if( ((100 * i) < csvArray[j][1]) && (csvArray[j][1] < (100 * i + 100)) ){
      daiNeigyo++;
    }
  }
...
quarter.textContent = csvArray[0][2];
...
for(let j = 1; j < 12; j++){
  ...
  // 実績売上
    ...
    act_sales = Number(elementTotal[0] / (Number(elementTotal[3]) / (csvArray.length - 1)));
    tdj.textContent = Math.round(act_sales).toLocaleString();
  }
  // 達成率
  else if( j == 4 ){
  ...
  // 累計差異
    ...
    tdj.textContent = Math.round((act_sales - Number(elementTotal[4])) * (Number(elementTotal[3]) / (csvArray.length - 1))).toLocaleString();
    if( act_sales - Number(elementTotal[4]) < 0 ){
      tdj.setAttribute("class", "TextRight TextColor-Red BorderRight-thin");
    }
    else{
  ...
  // 実績粗利
    ...
    act_GrossProfit = Number(elementTotal[1]) / (Number(elementTotal[3]) / (csvArray.length - 1));
    tdj.textContent = Math.round(act_GrossProfit).toLocaleString();
  }
  // 達成率
  else if( j == 9 ){
  ...
  // 平均差異
    ...
    tdj.textContent = Math.round((act_GrossProfit - Number(elementTotal[5])) * (Number(elementTotal[3]) / (csvArray.length - 1))).toLocaleString();
    if( act_GrossProfit - Number(elementTotal[5]) < 0 ){
      tdj.setAttribute("class", "TextRight TextColor-Red BorderRight");
    }
    else{
